...
const SERVICE_OPTIONS: ServiceOption[] = [
  // Retainer (deckungs­gleich zur Leistungsseite – Honorar ab 3.000 € / Monat)
  {
    id: "retainer",
...
export default function AnfragePage() {
  ...
  useEffect(() => {
    ...
    if (initial.length > 0) {
      setSelectedIds((prev) => (prev.length ? prev : Array.from(new Set(initial))));
    }
  ...
  // Rendering-Helfer: Kachel mit Honorar rechts (heller, größer, Abstand)
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="section-lead text-[var(--muted)] text-sm sm:text-base max-w-4xl">
              Auswahl der relevanten Module für Ihren Bedarf. Anschließend geben Sie Ihre Kontaktdaten und
              Entscheidungssituation an. Die Anfrage landet direkt in unserem Postfach: {" "}
          ...
          <div className="flex items-center gap-3 text-xs sm:text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className={step === 1 ? "text-white font-semibold" : "text-[var(--muted)]"}>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={step === 2 ? "text-white font-semibold" : "text-[var(--muted)]"}>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
            ...
            <section className="space-y-8">
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs sm:text-sm text-[var(--muted)] max-w-2xl">
                  Für Konstellationen mit laufenden Projekten, wiederkehrenden Abstimmungen
                  und kontinuierlichem Reportingbedarf.
                </p>

                <ServiceCard opt={SERVICE_OPTIONS.find((o) => o.id === "retainer")!} />
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {SERVICE_OPTIONS.filter((o) => o.group === "entwicklung").map((opt) => (
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {SERVICE_OPTIONS.filter((o) => o.group === "unternehmen").map((opt) => (
                    <ServiceCard key={opt.id} opt={opt} />
                  ))}
                </div>
              </div>
          ...
          {step === 2 && (
            <section className="space-y-8">
              ...
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)]/80 p-4 sm:p-5 space-y-3">
                ...
                {selectedOptions.length === 0 ? (
                  <p className="text-xs sm:text-sm text-[var(--muted)]">
                    Es sind aktuell keine Leistungen ausgewählt. Sie können oben im ersten Schritt Leistungen markieren.
                  </p>
                ) : (
                    ...
                    {selectedOptions.map((opt) => (
                      <li key={opt.id} className="flex items-start gap-2">
                        ...
                        <span className="text-white/90">
                          {opt.label}{" "}
                          <span className="text-[var(--muted)]">({opt.honorar})</span>
                        </span>
                      </li>
                    ))}
                  </ul>
              ...
              <div className="space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    <label className="text-xs sm:text-sm text-[var(--muted)]">Name*</label>
                    <input
                      name="name"
                      required
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    <label className="text-xs sm:text-sm text-[var(--muted)]">E-Mail*</label>
                    <input
                      type="email"
                      name="email"
                      required
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs sm:text-sm text-[var(--muted)]">Telefon (optional)</label>
                    <input
                      name="telefon"
                      className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)]/80 px-3 py-2 text-sm text-white outline-none focus:border-[var(--accent)]"
                    />
              ...
              {submitError && <p className="text-xs sm:text-sm text-red-400">{submitError}</p>}
